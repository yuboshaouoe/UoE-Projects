...
-- allFilled <&&> noneFilledTwice
-- <&&> squaresComplete
-- <&&> squaresNoRepetition
  
-- allFilled <&&> noneFilledTwice
-- <&&> rowsComplete <&&> columnsComplete <&&> rowsNoRepetition 
...
rowsComplete = And [ Or [ P (i, j, n) | i <- [1..9] ]
                   | j <- [1..9], n <- [1..9] ]

columnsComplete :: Form (Int,Int,Int)
columnsComplete = And [ Or [ P (i, j, n) | j <- [1..9] ]
                   | i <- [1..9], n <- [1..9] ]

squaresComplete :: Form (Int,Int,Int)
squaresComplete =  rowsComplete <&&> columnsComplete
...
squaresNoRepetition = And [ Or [ N (i, j, n), N (i, j', n), N (i, ja, n) ]
                       | i <- [1..9], n <- [1..9],
                         j <- [2,5,8], j' <- [1..(j-1)], ja <- [1..(j+1)]]
                      <&&>
                      And [ Or [ N (i, j, n), N (i', j, n), N (ia, j, n) ]
                       | j <- [1..9], n <- [1..9],
                         i <- [2,5,8], i' <- [1..(j-1)], ia <- [1..(j+1)]]
...
sudokuProblem = And [ Or [P (1,8,8)], Or [P (2,8,8)], Or [P (2,1,6)]
...
main = do
    putStrLn "Starting..."
    time $ time (printAllSolutions sudokuProblem) `seq` return ()
    putStrLn "Done."

-- by removing redundent constraints, the time it takes 
-- to complete the calculation can be shortened:

-- allFilled <&&> noneFilledTwice
-- <&&> rowsComplete <&&> columnsComplete <&&> squaresComplete
-- <&&> rowsNoRepetition <&&> columnsNoRepetition <&&> squaresNoRepetition
-- *Main> time (printAllSolutions sudokuProblem)
-- Computation time: 0.688 sec

-- allFilled <&&> noneFilledTwice
-- <&&> rowsComplete <&&> columnsComplete <&&> rowsNoRepetition 
-- <&&> columnsNoRepetition <&&> squaresNoRepetition
-- *Main> time (printAllSolutions sudokuProblem)
-- Computation time: 0.562 sec

-- allFilled <&&> noneFilledTwice
-- <&&> squaresComplete
-- <&&> squaresNoRepetition
-- *Main> time (printAllSolutions sudokuProblem)
-- Computation time: 0.422 sec