...
object Assignment3Embedded {
  ...
  object RabbitDSLImpl extends RabbitDSL {
    ...
    def moveXY( dx: RabbitAnimation[Int], dy: RabbitAnimation[Int]
              , a: RabbitAnimation[Frame]): RabbitAnimation[Frame]
      = lift3(a)(dx)(dy) { (f: Frame) => (i: Int) => (j: Int) =>
      f.map(p => Picture(name = p.name, x = p.x + i, y = p.y + j))
    }

      def scale(factor: RabbitAnimation[Double], a: RabbitAnimation[Frame])
             : RabbitAnimation[Frame]
      = lift2(a)(factor){(f: Frame) => (i: Double) =>
        f.map(p => Picture(name = p.name, x = (p.x * i).toInt, y = (p.y * i).toInt, scaleFactor =  p.scaleFactor * i))
      }

    def rotate(angle: RabbitAnimation[Double], a: RabbitAnimation[Frame])
              : RabbitAnimation[Frame]
      = lift2(a)(angle){(f: Frame) => (i: Double) =>
      f.map(p => Picture(name = p.name,
        x = (p.x * cos(i) + p.y * sin(i)).toInt,
        y = (-p.x * sin(i) + -p.y * cos(i)).toInt,
        angle = p.angle + i))
    }
